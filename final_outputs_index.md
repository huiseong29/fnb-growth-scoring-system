...
| ?�일 | ?�명 |
| --- | --- |
| `project_plan.md` | ?�체 ?�행 계획, ?�료 ?�태, ?�음 ?�업 |
| `project_proposal.md` | ?�로?�트 기획?? 문제 ?�의, 방법�? 결과 ?�석 |
| `remaining_tasks.md` | ?�으�??��? ?�업 체크리스??|
| `final_outputs_index.md` | ?�체 ?�출�??�덱??|
| `analysis_outputs/visualization_guide.md` | 모든 ?�각???�일??목적, ?�석, 발표??문장 |
| `score_design_rationale.md` | GroMong Score 5�?지?��? 가중치 ?�계 근거 |
| `midterm_quality_boost_plan.md` | 중간발표 1???�리?��? ?�한 보강 과제 로드�?|
...
| `score_design_rationale.md` | ?�코???�계 근거, 지?�별 ?�용 변?? ?�계?� 고도??방향 |
...
| ?�일 | ?�명 |
| --- | --- |
| `midterm_quality_boost_plan.md` | 부족한 부분과 보강 과제 ?�리 |
